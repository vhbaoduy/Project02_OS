...
/* 
Input: 
	- Khong gian dia chi cua User (int) -- virtAddr
 	- Gioi han cua buffer (int)  -- limit 
Output: - Bo nho dem Buffer (char*) 
Purpose: Sao chep vung nho tu User sang vung nho cua System
*/ 
char* Machine:: User2System(int virtAddr,int limit) 
{ 
	 int i;// index 
	 int oneChar; 
	 char* kernelBuffers = NULL; 
 
	 kernelBuffers = new char[limit +1];// them 1 phan tu ket thuc chuoi 
	 if (kernelBuffers == NULL) 
	 	return kernelBuffers; 
	 memset(kernelBuffers,0,limit+1); 
 
	 for (i = 0 ; i < limit ;i++) 
	 { 
		 ReadMem(virtAddr+i,1,&oneChar); 
		 kernelBuffers[i] = (char)oneChar; 

		 if (oneChar == 0) // phan tu ket thuc chuoi \0
		 break; 
	 } 
	 return kernelBuffers; 
}
...
Input: 
	- Khong gian dia chi cua User (int) -- virtAddr
 	- Gioi han cua buffer (int)  -- limit 
	- Bo nho dem Buffer (char[]) --buffer
Output:- So bytes da sao chep duoc (int)
Purpose: Sao chep vung nho tu System sang vung nho cua System
*/ 
